...
| — | M03 | deployed | *(re-run pending after ADR-014/015 — `make deploy-dev`, then `make eval` and `make eval-adversarial`; needs the scorecard pass rate and the `--diff` path, which has still never executed)* | — | — |
