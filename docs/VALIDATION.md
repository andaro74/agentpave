...
- [ ] **Deployed gate, run by a human.** Two things, neither yet done.
      `make shadow-eval` ran once on 2026-08-13 and **does not count**: the
      gateway predated the candidate feature, both arms served on Haiku, and the
      report said the candidate was safe to adopt. **`make deploy-dev` first** —
      the routing table is deployed code, and a shadow run against a stale
      gateway compares the incumbent to itself. The re-run costs another ~$0.94.
      Then Act 3 — stage a schema change, let the contract suite redden,
...
- ~~**The shadow candidate has never reached Bedrock.**~~ **It still has not,
  and the first attempt proved the point the hard way** — see the 2026-08-13
  deployed row. The report now refuses instead of reassuring: both arms observing
  the same model is incomparable, not shippable. **Deploy before running it.**
  The remaining unknown is unchanged — nothing has yet confirmed that
  `shadow-candidate` reaches Sonnet through a deployed gateway, only that the
  report will now say so if it does not.
...
- **The golden set has no headroom, and this is the finding M07 must carry.**
  The incumbent passes 31/31, so a shadow run can only ever say "no change" or
  "regression" — a candidate that is genuinely *better* has nowhere to show it.
  ARCHITECTURE §7 Q3 asks at what dataset size the golden set stops being a
  meaningful canary stand-in; the deployed run says the binding constraint is
  not size but saturation. Answering it properly needs cases the incumbent
  fails, which is a dataset decision rather than a code one.
