...
- [ ] **Deployed gate, half two — Act 3.** Stage a schema change, let the
      contract suite redden, run `pave selfheal`, run Claude Code against its
      verdict from a laptop, and merge the resulting `ai-proposed` pull request
      through `gate verdict`.
- [ ] `M06` tag.
